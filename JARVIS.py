...
def sendEmail(to, content):
    ...
    server.ehlo()
    server.starttls()
    server.login('anish28032002@gmail.com', 'abirpal28')
    ...
    # if 1:
        ...
        #Greeting 
            ...
            speak("wow, thats Great Sir! ")
                ...
                response = requests.get(complete_url)
                speak(f'Here is the weater of {query_new} ')
                x = response.json()
                if x["cod"] != "404": 
                    y = x["main"]
                    current_temperature = y["temp"]
                    ...
                    speak(f"And Sir, the Air Presser is {current_pressure} pascal in {query_new} ")
                else: 
                    speak(" City Not Found ")
